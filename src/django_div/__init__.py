...
def render_attrs(attrs: dict[str, Any]) -> str:
    ...
    for raw_name, value in attrs.items():
        ...
        if name.startswith("aria-") and isinstance(value, bool):
...
class Tag(_Container):
    ...
    def with_attrs(self, /, **attrs: Any) -> Tag:
        """Return a shallow copy with normalized attribute values replaced.

        Attributes and children have independent containers; existing child
        objects and nested attribute values are shared. Classes are replaced,
        not merged. False and None retain their usual rendering behavior.
        """
        clone = self.model_copy()
        clone.attrs = {**self.attrs, **{normalize_attr(k): v for k, v in attrs.items()}}
        clone.children = list(self.children)
        # As in construction, retain one-shot class values before rendering
        # or serialization can consume them.
        for name in clone.attrs:
            if normalize_attr(name) == "class":
                _class_value(clone.attrs, name)
        return clone
...
class JsonScript(Script):  # noqa: F821 - Script is generated by the loop above
    """A script holding JSON data for JavaScript to read with JSON.parse.

    Models are dumped in JSON mode with aliases and null values preserved.
    Keywords become attributes; the default type is application/json.
    Parsing or loading through Tag restores a plain Script.
    """
    ...
    def __init__(self, data: Any = None, **attrs: Any) -> None:
        ...
        content = json.dumps(
            ...
            separators=(",", ":"),
            allow_nan=False,
        ).translate(JSON_LD_ESCAPES)
        Tag.__init__(self, "script", content, **{"type": "application/json", **attrs})
